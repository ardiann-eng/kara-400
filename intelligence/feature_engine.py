...
def extract_live_features(score: int, meta_delta: int, bd, funding_rate: float, realized_vol: float, trend_pct: float, side: str = "long") -> list:
    """
    ...
    [score, meta_delta, oi_score, liq_score, ob_score, session_bonus, funding_rate, realized_vol, trend_pct, is_short]
    """
    try:
        ...
        return [
            ...
            float(realized_vol),
            float(trend_pct),
            float(1 if str(side).lower() == "short" else 0)  # side encoding: SHORT=1, LONG=0
        ]
